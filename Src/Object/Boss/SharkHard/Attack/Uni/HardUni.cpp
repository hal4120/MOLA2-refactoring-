...
HardUni::HardUni(int* img,int arrowImg) :
	img_(),
	arrowImg_(arrowImg),
...
#define SET_STATE(state, func) stateFuncPtr[(int)(state)] = static_cast<void (HardUni::*)(void)>(func)
	SET_STATE(STATE::SUMMON, &HardUni::Summon);
	SET_STATE(STATE::THROWN, &HardUni::Thrown);
	SET_STATE(STATE::REFLECTION, &HardUni::Reflection);
	SET_STATE(STATE::BLAST, &HardUni::Blast);
}

void HardUni::Update(void)
{
	arrow_ = false;
...
void HardUni::OnCollision(UnitBase* other)
{
	if (state_ == STATE::NON || state_ == STATE::BLAST) { return; }
	...
	if ((dynamic_cast<EnemyBase*>(other) || dynamic_cast<HardUni*>(other))
		&& unit_.para_.colliType == CollisionType::ENEMY) {
		state_ = STATE::REFLECTION;
		unit_.para_.colliType = CollisionType::ALLY;
		Vector2 vec = unit_.pos_ - other->GetUnit().pos_;
...
void HardUni::On(Vector2 pos, Vector2 target)
{
	state_ = STATE::SUMMON;
