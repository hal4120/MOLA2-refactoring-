...
HardMizu::HardMizu(const Vector2& bossPos, const Vector2& playerPos):
	bossPos_(bossPos),
...
void HardMizu::Update(void)
{
	...
	scaleUp_ += 0.001f;
	unit_.para_.radius = (SIZE / 2) * ((SCALE + scaleUp_) / SCALE);
...
void HardMizu::Draw(void)
{
	if (!unit_.isAlive_) { return; }
	DrawRotaGraph3F(unit_.pos_.x, unit_.pos_.y, CENTER_X, CENTER_Y, SCALE + scaleUp_, SCALE + scaleUp_, angle_, img_, true);
	if (arrow_) { DrawRotaGraphF(unit_.pos_.x, unit_.pos_.y, 1, arrowAngle_, arrowImg_, true); }
}

void HardMizu::Release(void)
{
	DeleteGraph(img_);
}
